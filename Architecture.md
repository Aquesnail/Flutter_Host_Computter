...
- **自适应横轴**：横轴刻度与网格根据缩放级别自动适配，缩小时用秒（s），放大时自动切换毫秒（ms），并增加次级刻度和网格线以显示更细微的时间尺度
...
**支持的命令字**: `0x00` 握手、`0x5A` 修改变量、`0x56` 动态注册、`0x57` 文本、`0x58` 请求刷新静态变量。
...
| 操作 | 行为 |
|------|------|
| 修改变量值 | 发送写命令 (0x5A)，如为静态变量则**自动**请求刷新 |
| 点击刷新按钮 | 发送刷新请求 (0x58) |
...
### 8.1 功能概述
车辆操控工具是一个独立的浮动面板，用于通过上位机实时控制两轮差速小车。支持两种操控模式：变量写入模式（直接写入速度变量）和指令模式（发送文本命令由下位机解析）。

### 7.2 浮动面板 (`vehicle_control_window.dart`)
- 全局 `OverlayEntry` 单例，与主界面独立，可自由拖拽和折叠
- 拖拽标题栏可移动位置，折叠/展开按扭控制内容可见性
- 关闭按钮移除 overlay 并释放资源

### 7.3 操控模式
| 模式 | 原理 | 适用场景 |
|------|------|---------|
| 变量写入 | 检测 `R_Speed` / `L_Speed` 变量，WASD 写入速度值 | 变量直驱，上位机计算轮速 |
| 指令模式 | 发送 `forward`/`backward`/`left`/`right`/`stop` 文本命令 (CMD 0x57) | 由下位机 `Motor_HandleCommand` 解析执行 |

### 7.4 两轮差速映射
| 按键 | 左轮 (L_Speed) | 右轮 (R_Speed) | 效果 |
|------|---------------|---------------|------|
| W | +前进速度 | +前进速度 | 前进 |
| S | -后退速度 | -后退速度 | 后退 |
| A | -转向速度 | +转向速度 | 左转 |
...
按下一个键写入对应速度值，松开该键写入 0（停止）。同一时刻只响应一个按键，防止冲突。

### 7.5 配置持久化
- 速度值（前进/转向/后退）和操控模式保存到 `~/.flowave/vehicle_control_config.json`
- 启动时自动加载上次配置

### 7.6 键盘处理
- 使用 `Focus` + `onKeyEvent` 捕获 WASD 按键
- 开始操控时取消文本框焦点，防止输入冲突
- `KeyUpEvent` 时恢复零速/发送 stop 命令
...
### 9.1 数组检测与分组
- 同名（忽略大小写）静态变量自动归并为数组，按注册 ID 升序排列
- `StaticVarGroup` 模型统一处理单个变量和数组变量
- `buildStaticVarGroups(registry)` 函数执行分组逻辑

### 9.2 数组编辑器 (`ArrayEditorDialog`)
- 弹出对话框中以表格形式批量编辑数组元素
- 每行：索引标签 | 可编辑值字段 | 类型标签 | 刷新/写入按钮
- 底部操作：全部刷新 | 全部写入 | 关闭
- 500ms 定时轮询 registry，自动同步显示值

### 9.3 浮动多列窗口 (`static_vars_window.dart`)
- 全局 `OverlayEntry` 单例（独立于车辆操控 overlay）
- 紫色主题拖拽标题栏 + 折叠/展开/关闭
- **Wrap 多列网格**：紧凑卡片（宽 ~185px），2-3 列自适应
- 单变量卡片：名称、ID 徽章、等宽大字号值、类型标签、刷新按钮 → 点击编辑
- 数组卡片：`name [0..N]`、元素数量徽章、值摘要 → 点击打开数组编辑器
- 操作栏：全部刷新 / 全部写入 / 导入 / 导出
- 未连接时显示橙色警告栏
- 500ms 轮询 + 缓存比较，避免无效重建

### 9.4 共享代码 (`static_vars_actions.dart`)
- 操作函数：`refreshAllStaticVars`、`writeAllStaticVars`、`importStaticVarsFromJson`、`exportStaticVarsFromJson`
- 工具函数：`varTypeLength`、`parseVarValue`（支持十进制/十六进制/浮点）
- `StaticVarGroup` 模型和 `buildStaticVarGroups` 分组函数
- `ArrayEditorDialog` 供面板和窗口复用
...
└── ui/
    ...
    │   ├── scope_dashboard.dart
	    │   ├── static_vars_actions.dart
	    │   ├── static_vars_panel.dart
	    │   └── static_vars_window.dart
    └── scope/
