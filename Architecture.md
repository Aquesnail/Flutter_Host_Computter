...
│       └── device_controller.dart  # Flutter 包装器
└── ui/
    ├── main_window.dart
    ├── widgets/
...
### 7.1 概述
flowave 是配套的命令行工具，用于串口调试、变量操作和实时监控。编译为独立可执行文件，不依赖 Flutter 环境。

### 7.2 命令列表
| 命令 | 说明 |
|------|------|
| `list-ports` | 列出可用串口 |
| `connect --port <name>` | 连接串口 |
| `disconnect` | 断开连接 |
| `handshake` | 握手 |
| `register <addr> <name> <type>` | 注册变量 |
| `write <varId> <value> <type>` | 修改变量 |
| `refresh <varId>` | 刷新静态变量 |
| `refresh-all` | 刷新所有静态变量 |
| `list-vars` | 列出变量 |
| `save-static <path>` | 导出静态变量 |
| `load-static <path>` | 导入静态变量 |
| `write-all-static` | 批量写入静态变量 |
| `text <msg>` | 发送文本 |
| `monitor [--highfreq] [--log]` | 监控流 |
| `stats <varId>` | 统计分析 |
| `plot <varId>` | 绘制波形 |
